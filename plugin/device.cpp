...
*/
    // Begin of Qt6 Add
    // If the user supplied BitRateKey…
    if (configurationKeys().contains(QCanBusDevice::BitRateKey)) {
        bool ok = false;
        uint32_t bitrate = configurationParameter(QCanBusDevice::BitRateKey)
                               .toUInt(&ok);      // QVariant::toUInt :contentReference[oaicite:3]{index=3}
        if (ok && !setBitTiming(bitrate, 875)) {
            close();
            return false;
        }
    }
